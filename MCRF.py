...
'''
参考：
https://github.com/DandyQi/MaskedCRF/blob/master/masked_crf.py
'''
...
class CRF(nn.Module):
    ...
    def _compute_normalizer(
        ...
        if self.label2idx_map is not None:
            mask_trans = self.get_mask_trans()
            with torch.no_grad():
                for i in range(mask_trans.shape[0]):
                    for j in range(mask_trans.shape[1]):
                        if mask_trans[i][j] == -100:
                            self.transitions[i][j] = -100
